...
public class SCAB_SGA {
    ...
    public static class SGA{
        ...
        public static double[][][] p,prep,v,prev;
        ...
        public static void fitness(){
            for(int s = 0; s < popSize; s++){
                ...
                if(pbestFitness > fi){
                    pbestFitness = fi;
                    for(int x = 1; x <= 24; x++){
                        if (N_TGU + N_CHP * 2 + N_HOU >= 0)
                            System.arraycopy(p[s][x], 0, pbest[x], 0, N_TGU + N_CHP * 2 + N_HOU);
                    }
                }
                fitVal[s] = fi;
            }
        }
        public static void updateV(int its, double xita) {
            for (int s = 0; s < popSize; s++) {
                // 更新H
                for(int t = 1; t <= 24; t++){
                    for(int i = N_TGU; i < Dim; i++){
                        if(i < N_TGU + N_CHP*2) i++;
                        v[s][t][i] = 4 * ((double) its / T) / Math.pow(Math.E, (4 * (double) its / T)) * v[s][t][i] + (pbest[t][i] - p[s][t][i] - 1.29 * v[s][t][i] * v[s][t][i] * Math.sin(xita)) * 0.01;
                        if(Math.abs(v[s][t][i]) > P_H_MAX[s][t][i]) {
                            v[s][t][i] = random.nextDouble()*P_H_MAX[s][t][i]*(v[s][t][i] > 0 ? 1 : -1);
                        }
                        else if (Math.abs(v[s][t][i]) < P_H_MAX[s][t][i]/T && random.nextDouble() < 0.8) {
                            v[s][t][i] *= 10;
                        }
                        p[s][t][i] = Math.min(Math.max(p[s][t][i] + v[s][t][i], P_H_MIN[s][t][i]), P_H_MAX[s][t][i]);
                    }
                ...
                for(int t = 1; t <= 24; t++){
                    ...
                    for(int i = 0; i < Dim - N_HOU; i++){
                        ...
                        else if (Math.abs(v[s][t][i]) < P_H_MAX[s][t][i]/T*r3 && random.nextDouble() < r1) {
                            v[s][t][i] *= random.nextInt(5,10);
                        }
                        if(T > 2000 && v[s][t][i] > 0 && random.nextDouble() < 0.2) v[s][t][i] = -v[s][t][i]; // 加速收敛
                        p[s][t][i] = Math.min(Math.max(p[s][t][i] + v[s][t][i], P_H_MIN[s][t][i]), P_H_MAX[s][t][i]);
                        if(t >= 2){
                            if(i < N_TGU){
                                ...
                                if(p[s][t][i] - p[s][t-1][i] > 0) p[s][t][i] = Math.min(p[s][t][i], p[s][t-1][i] + UR_CHP[b]);
                                else p[s][t][i] = Math.max(p[s][t][i],p[s][t-1][i]-DR_CHP[b]);
                                b++;
                            }
                        }
                        if(i >= N_TGU) i++;
        ...
        public static void updatePStageOne(double b, double d, double e){
            double[][] pc = new double[1+24][Dim];
            double sum = Arrays.stream(fitVal).sum()*popSize;

            for(int s = 0; s < popSize; s++){
                for(int t = 1; t <= 24; t++){
                    for(int i = 0; i < Dim; i++){
                        pc[t][i] += fitVal[s]*p[s][t][i];
                        if(s == popSize -1) pc[t][i] = pc[t][i]/sum;
                    }
                }
            }
            for(int s = 0; s < popSize; s++){
                ...
                for(int t = 1; t <= 24; t++){
                    for(int i = N_TGU; i < Dim; i++){
                        if(i < N_TGU + N_CHP * 2) i++;
                        if(s < popSize*0.2){
                            p[s][t][i] = Math.min(Math.max(p[s][t][i] + b*(pbest[t][i]-p[s][t][i]) + v[s][t][i], P_H_MIN[s][t][i]), P_H_MAX[s][t][i]);
                        } else if (s > popSize*0.8) {
                            p[s][t][i] = Math.min(Math.max(p[s][t][i] + b*(pbest[t][i]-p[s][t][i]) + d*(pc[t][i]-p[s][t][i]) + v[s][t][i], P_H_MIN[s][t][i]), P_H_MAX[s][t][i]);
                        }else {
                            p[s][t][i] = Math.min(Math.max(p[s][t][i] + b*(pbest[t][i]-p[s][t][i]) + d*(pc[t][i]-p[s][t][i]) - e*(p[popSize-1][t][i]+p[s][t][i]) + v[s][t][i], P_H_MIN[s][t][i]), P_H_MAX[s][t][i]);
                        }
                    }
                    if(constrainH(s,t)) updateH(s,t);
                    else backPreH(s,t);
        ...
        public static double brown(int its){
            double d = 0;
            for(int i = 0; i < N_TGU + N_CHP*2 + N_HOU; i++) d+= random.nextGaussian(0,2*(Math.cos((double) (its%100) / 100 * (Math.PI/2)) + r4));
            return Math.sqrt((1.0d/(N_TGU + N_CHP*2 + N_HOU)))*d;
        }
        public static void updatePStageTwo(int its){
            for(int s = 0; s < popSize; s++){
                double o = brown(its);
                // 更新H
                for(int t = 1; t <= 24; t++){
                    for(int i = N_TGU; i < Dim; i++){
                        ...
                        if(r > 0.5){
                            p[s][t][i] = Math.min(Math.max(p[s][t][i] + (p[s][t][i] - pbest[t][i])*r, P_H_MIN[s][t][i]), P_H_MAX[s][t][i]);
                        }else {
                            p[s][t][i] = Math.min(Math.max(pbest[t][i] + (p[s][t][i]-pbest[t][i])*r*o, P_H_MIN[s][t][i]), P_H_MAX[s][t][i]);
                        }
                    }
                    if(constrainH(s,t)) updateH(s,t);
                    else backPreH(s,t);
        ...
        public static void init() {
            ...
            v = new double[popSize][1+24][Dim];prev = new double[popSize][1+24][Dim];
            ...
            for(int s = 0; s < popSize; s++){
                ...
                for(int t = 1; t <= 24; t++){
                    for(int i = 0; i < Dim; i++){
                        prep[s][t][i] = p[s][t][i];
                        v[s][t][i] = random.nextDouble(P_H_MIN[s][t][i]/10, P_H_MIN[s][t][i]/10 + (P_H_MAX[s][t][i]-P_H_MIN[s][t][i])/10);
                        prev[s][t][i] = v[s][t][i];
                    }
                }
            }
        ...
        public void start(String filename) {
            ...
            for(int its = 1; its <= T; its++){
                ...
                if(xita <= Math.PI){
                    updatePStageOne(4*random.nextDouble(0,1)-2, 3*random.nextDouble(0,1)-1.5, 2*random.nextDouble(0,1)-1);
                }
                else {
                    updatePStageTwo(its);
                }
                fitness();
                System.out.println("iterations : " + its + " / " + T + "  | minFit : " + (pbestFitness + windCost + SpvpCost));
                try {
                    assert foss != null;
    ...
    public static void main(String[] args) {
        ...
        double[] r1 = {0.8,0.8,0.8}, r3 = {2,2,6}, r4 = {0.1,0,0}, T = {1000,1000,3000};
        for(int i = 0; i < r1.length; i++){
            for(int eachtime = 0 ; eachtime < 30; eachtime++){
                SGA sga = new SGA(100, (int) T[i],r1[i],r3[i],r4[i]);
