...
//            return 2*Test.get("WTG_Oe",0,p_WTG[s][t][i]) + Test.get("WTG_Ue", p_WTG[s][t][i], P_WTG_MAX[t][i]);
        ...
        public static void updateStageOne(double r1, double r2, double r3, double r4){
            for(int s = 0; s < popSize; s++){
                // H
                for(int t = 1; t <= 24; t++){
                    for(int i = 0; i < N_CHP; i++){
                        if(r4 > 0.5) {
                            h_CHP[s][t][i] = Math.min(Math.max(h_CHP[s][t][i]+r1*Math.sin(r2)*Math.abs(r3*best_h_CHP[t][i]-h_CHP[s][t][i]), H_CHP_MIN[i]), H_CHP_MAX[i]);
                            p_P2G[s][t][i] = Math.min(Math.max(p_P2G[s][t][i]+r1*Math.sin(r2)*Math.abs(r3*best_P2G[t][i]-p_P2G[s][t][i]), 0), P_P2G_MAX[i]);
                        }
                        else{
                            h_CHP[s][t][i] = Math.min(Math.max(h_CHP[s][t][i]+r1*Math.cos(r2)*Math.abs(r3*best_h_CHP[t][i]-h_CHP[s][t][i]), H_CHP_MIN[i]), H_CHP_MAX[i]);
                            p_P2G[s][t][i] = Math.min(Math.max(p_P2G[s][t][i]+r1*Math.cos(r2)*Math.abs(r3*best_P2G[t][i]-p_P2G[s][t][i]), 0), P_P2G_MAX[i]);
                        }

                    }
                    for(int i = 0; i < N_HOU; i++){
                        if(r4 > 0.5) {
                            h_HOU[s][t][i] = Math.min(Math.max(h_HOU[s][t][i]+r1*Math.sin(r2)*Math.abs(r3*best_HOU[t][i]-h_HOU[s][t][i]), H_HOU_MIN[i]), H_HOU_MAX[i]);
                        }
                        else{
                            h_HOU[s][t][i] = Math.min(Math.max(h_HOU[s][t][i]+r1*Math.cos(r2)*Math.abs(r3*best_HOU[t][i]-h_HOU[s][t][i]), H_HOU_MIN[i]), H_HOU_MAX[i]);
                        }
                    }
                ...
                for(int t = 1; t <= 24; t++){
                    for(int i = 0; i < N_TGU; i++){
                        if(r4 > 0.5) {
                            p_TGU[s][t][i] = Math.min(Math.max(p_TGU[s][t][i]+r1*Math.sin(r2)*Math.abs(r3*best_TGU[t][i]-p_TGU[s][t][i]), P_TGU_MIN[i]), P_TGU_MAX[i]);
                        }
                        else{
                            p_TGU[s][t][i] = Math.min(Math.max(p_TGU[s][t][i]+r1*Math.cos(r2)*Math.abs(r3*best_TGU[t][i]-p_TGU[s][t][i]), P_TGU_MIN[i]), P_TGU_MAX[i]);
                        }
                    ...
                    for(int i = 0; i < N_CHP; i++){
                        if(r4 > 0.5) {
                            p_CHP[s][t][i] = Math.min(Math.max(p_CHP[s][t][i]+r1*Math.sin(r2)*Math.abs(r3*best_p_CHP[t][i]-p_CHP[s][t][i]), P_CHP_CUR_MIN[s][t][i]), P_CHP_CUR_MAX[s][t][i]);
                        }
                        else{
                            p_CHP[s][t][i] = Math.min(Math.max(p_CHP[s][t][i]+r1*Math.cos(r2)*Math.abs(r3*best_p_CHP[t][i]-p_CHP[s][t][i]), P_CHP_CUR_MIN[s][t][i]), P_CHP_CUR_MAX[s][t][i]);
                        }
                        if(t >= 2){
                    ...
                    for(int i = 0; i < N_SPVP; i++){
                        if(r4 > 0.5) {
                            p_SPVP[s][t][i] = Math.min(Math.max(p_SPVP[s][t][i]+r1*Math.sin(r2)*Math.abs(r3*best_SPVP[t][i]-p_SPVP[s][t][i]), 0), P_SPVP_MAX[t][i]);
                        }
                        else{
                            p_SPVP[s][t][i] = Math.min(Math.max(p_SPVP[s][t][i]+r1*Math.cos(r2)*Math.abs(r3*best_SPVP[t][i]-p_SPVP[s][t][i]), 0), P_SPVP_MAX[t][i]);
                        }
                    }
                    for(int i = 0; i < N_WTG; i++){
                        if(r4 > 0.5) {
                            p_WTG[s][t][i] = Math.min(Math.max(p_WTG[s][t][i]+r1*Math.sin(r2)*Math.abs(r3*best_WTG[t][i]-p_WTG[s][t][i]), 0), P_WTG_MAX[t][i]);
                        }
                        else{
                            p_WTG[s][t][i] = Math.min(Math.max(p_WTG[s][t][i]+r1*Math.cos(r2)*Math.abs(r3*best_WTG[t][i]-p_WTG[s][t][i]), 0), P_WTG_MAX[t][i]);
                        }
                    }
                    if(!constrainP(s,t)){
                        backPreP(s);
        ...
        public void start(String filename) {
            ...
            try {
                foss = new FileOutputStream("SCAData2.txt",true);
            }catch (Exception ignored){}
            init();
            double a = 2;
            for(int its = 1; its <= T; its++){
                myQuicksort(0,popSize-1);
                updateStageOne(a - a*its/T,random.nextDouble()*2*Math.PI, random.nextDouble(0,10), random.nextDouble());
                fitness();
    ...
    public static void main(String[] args) {
        for(int eachtime = 0 ; eachtime < 30; eachtime++){
            SCA sca = new SCA(60, 5000);
            String filename = "SCAData1.txt";
            sca.start(filename);
        }
    }
}
