...
public class PSOALG {
    ...
    public static class PSO{
        ...
        public static void updateV(double c1, double c2) {
            for (int s = 0; s < popSize; s++) {
                ...
                for(int t = 1; t <= 24; t++){
                    for(int i = N_TGU; i < Dim; i++){
                        if(i < N_TGU + N_CHP*2) i++;
                        v[s][t][i] = v[s][t][i] + c1*random.nextDouble()*(p[0][t][i] - p[s][t][i]) + c2* random.nextDouble()*pbest[t][i];
                    }
                }
            }
        }
        public static void updatePStage(){
            for(int s = 0; s < popSize; s++){
                // 更新H
                for(int t = 1; t <= 24; t++){
                    for(int i = N_TGU; i < Dim; i++){
        ...
        public void start(String filename) {
            ...
            try {
                foss = new FileOutputStream("C:\\Users\\Qiu\\Desktop\\EconomicDispatch\\result5\\system4\\bestits.txt",true);
            }catch (Exception e){
    ...
    public static void main(String[] args) {
        for(int eachtime = 0 ; eachtime < 30; eachtime++){
            PSO pso = new PSO(100,1000);
            String filename = "C:\\Users\\Qiu\\Desktop\\EconomicDispatch\\result5\\system4\\data.txt";
            pso.start(filename);
        }
    }
}
