...
//            return 2*Test.get("WTG_Oe",0,p_WTG[s][t][i]) + Test.get("WTG_Ue", p_WTG[s][t][i], P_WTG_MAX[t][i]);
        ...
        public static void updateV(double c1, double c2){
            for(int s = 0; s < popSize; s++){
                for(int t = 1; t <= 24; t++){
                    // H
                    for(int i = 0; i < N_CHP; i++){
                        v_h_CHP[s][t][i] = v_h_CHP[s][t][i] + c1*random.nextDouble()*(h_CHP[0][t][i] - h_CHP[s][t][i]) + c2* random.nextDouble()*best_h_CHP[t][i];
                        v_P2G[s][t][i] = v_P2G[s][t][i] + c1*random.nextDouble()*(p_P2G[0][t][i] - p_P2G[s][t][i]) + c2* random.nextDouble()*best_P2G[t][i];
                    }
                    for(int i = 0; i < N_HOU; i++){
                        v_HOU[s][t][i] = v_HOU[s][t][i] + c1*random.nextDouble()*(h_HOU[0][t][i] - h_HOU[s][t][i]) + c2* random.nextDouble()*best_HOU[t][i];
                    }
                    // P
                    for(int i = 0; i < N_TGU; i++){
                        v_TGU[s][t][i] = v_TGU[s][t][i] + c1*random.nextDouble()*(p_TGU[0][t][i] - p_TGU[s][t][i]) + c2* random.nextDouble()*best_TGU[t][i];
                    }
                    for(int i = 0; i < N_CHP; i++){
                        v_p_CHP[s][t][i] = v_p_CHP[s][t][i] + c1*random.nextDouble()*(p_CHP[0][t][i] - p_CHP[s][t][i]) + c2* random.nextDouble()*best_p_CHP[t][i];
                    }
                    for(int i = 0; i < N_SPVP; i++){
                        v_SPVP[s][t][i] = v_SPVP[s][t][i] + c1*random.nextDouble()*(p_SPVP[0][t][i] - p_SPVP[s][t][i]) + c2* random.nextDouble()*best_SPVP[t][i];
                    }
                    for(int i = 0; i < N_WTG; i++){
                        v_WTG[s][t][i] = v_WTG[s][t][i] + c1*random.nextDouble()*(p_WTG[0][t][i] - p_WTG[s][t][i]) + c2* random.nextDouble()*best_WTG[t][i];
                    }
                }
            }
        }
        ...
        public void start(String filename) {
            ...
            try {
                foss = new FileOutputStream("PSOData2.txt",true);
