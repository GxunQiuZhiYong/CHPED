...
public class PIO {
    ...
    public static class PIO{
        ...
        public PIO(int popSize, int T, double r1, double r3, double r4){
            PIO.popSize = popSize;
            PIO.T = T;
        }
...
//            return 2*Test.get("WTG_Oe",0,p_WTG[s][t][i]) + Test.get("WTG_Ue", p_WTG[s][t][i], P_WTG_MAX[t][i]);
        ...
        public static void updateV(double its){
            for(int s = 0; s < popSize; s++){
                for(int t = 1; t <= 24; t++){
                    // H
                    for(int i = 0; i < N_CHP; i++){
                        v_h_CHP[s][t][i] = Math.pow(Math.E, -0.2*its)*v_h_CHP[s][t][i] + random.nextDouble()*(best_h_CHP[t][i] - h_CHP[s][t][i]);
                    }
                    for(int i = 0; i < N_HOU; i++){
                        v_HOU[s][t][i] = Math.pow(Math.E, -0.2*its)*v_HOU[s][t][i] + random.nextDouble()*(best_HOU[t][i] - h_HOU[s][t][i]);
                    }
                    // P
                    for(int i = 0; i < N_TGU; i++){
                        v_TGU[s][t][i] = Math.pow(Math.E, -0.2*its)*v_TGU[s][t][i] + random.nextDouble()*(best_TGU[t][i] - p_TGU[s][t][i]);
                    }
                    for(int i = 0; i < N_CHP; i++){
                        v_p_CHP[s][t][i] = Math.pow(Math.E, -0.2*its)*v_p_CHP[s][t][i] + random.nextDouble()*(best_p_CHP[t][i] - p_CHP[s][t][i]);
                        v_P2G[s][t][i] = Math.pow(Math.E, -0.2*its)*v_P2G[s][t][i] + random.nextDouble()*(best_P2G[t][i] - p_P2G[s][t][i]);
                    }
                    for(int i = 0; i < N_SPVP; i++){
                        v_SPVP[s][t][i] = Math.pow(Math.E, -0.2*its)*v_SPVP[s][t][i] + random.nextDouble()*(best_SPVP[t][i] - p_SPVP[s][t][i]);
                    }
                    for(int i = 0; i < N_WTG; i++){
                        v_WTG[s][t][i] = Math.pow(Math.E, -0.2*its)*v_WTG[s][t][i] + random.nextDouble()*(best_WTG[t][i] - p_WTG[s][t][i]);
                    }
                }
            }
        }
        public static void updateStageOne(){
            for(int s = 0; s < popSize; s++){
                // H
                for(int t = 1; t <= 24; t++){
                    for(int i = 0; i < N_CHP; i++){
                        h_CHP[s][t][i] = Math.min(Math.max(h_CHP[s][t][i] + v_h_CHP[s][t][i], H_CHP_MIN[i]), H_CHP_MAX[i]);
                        p_P2G[s][t][i] = Math.min(Math.max(p_P2G[s][t][i] + v_P2G[s][t][i], 0), P_P2G_MAX[i]);
                    }
                    for(int i = 0; i < N_HOU; i++){
                        h_HOU[s][t][i] = Math.min(Math.max(h_HOU[s][t][i] + v_HOU[s][t][i], H_HOU_MIN[i]), H_HOU_MAX[i]);
                    }
                ...
                for(int t = 1; t <= 24; t++){
                    for(int i = 0; i < N_TGU; i++){
                        p_TGU[s][t][i] = Math.min(Math.max(p_TGU[s][t][i] + v_TGU[s][t][i], P_TGU_MIN[i]), P_TGU_MAX[i]);
                        if(t >= 2){
                    ...
                    for(int i = 0; i < N_CHP; i++){
                        p_CHP[s][t][i] = Math.min(Math.max(p_CHP[s][t][i] + v_p_CHP[s][t][i], P_CHP_CUR_MIN[s][t][i]), P_CHP_CUR_MAX[s][t][i]);
                        if(t >= 2){
                    ...
                    for(int i = 0; i < N_SPVP; i++){
                        p_SPVP[s][t][i] = Math.min(Math.max(p_SPVP[s][t][i] + v_SPVP[s][t][i], 0), P_SPVP_MAX[t][i]);

                    }
                    for(int i = 0; i < N_WTG; i++){
                        p_WTG[s][t][i] = Math.min(Math.max(p_WTG[s][t][i] + v_WTG[s][t][i], 0), P_WTG_MAX[t][i]);
                    }
                    if(!constrainP(s,t)){
                        backPreP(s);
                        flag = false;
        ...
        public static void updateStageTwo(){
            ...
            if(Np > popSize * 0.2) Np /= 2;
            for(int s = 0; s < popSize; s++){
                // H
                for(int t = 1; t <= 24; t++){
                    for(int i = 0; i < N_CHP; i++){
                        h_CHP[s][t][i] = Math.min(Math.max(h_CHP[s][t][i] + random.nextDouble()*(pc_h_CHP[t][i] - h_CHP[s][t][i]), H_CHP_MIN[i]), H_CHP_MAX[i]);
                        p_P2G[s][t][i] = Math.min(Math.max(p_P2G[s][t][i] + random.nextDouble()*(pc_P2G[t][i] - p_P2G[s][t][i]), 0), P_P2G_MAX[i]);
                    }
                    for(int i = 0; i < N_HOU; i++){
                        h_HOU[s][t][i] = Math.min(Math.max(h_HOU[s][t][i] + random.nextDouble()*(pc_HOU[t][i] - h_HOU[s][t][i]), H_HOU_MIN[i]), H_HOU_MAX[i]);
                    }
                    if(constrainH(s,t)) updateH(s,t);
                    else backPreH(s,t);
                }
                ...
                for(int t = 1; t <= 24; t++){
                    for(int i = 0; i < N_TGU; i++){
                        p_TGU[s][t][i] = Math.min(Math.max(p_TGU[s][t][i] + random.nextDouble()*(pc_TGU[t][i] - p_TGU[s][t][i]), P_TGU_MIN[i]), P_TGU_MAX[i]);
                        if(t >= 2){
                            if(p_TGU[s][t][i] - p_TGU[s][t-1][i] > 0) p_TGU[s][t][i] = Math.min(p_TGU[s][t][i], p_TGU[s][t-1][i] + UR_TGU[i]);
                            else p_TGU[s][t][i] = Math.max(p_TGU[s][t][i],p_TGU[s][t-1][i]-DR_TGU[i]);
                    ...
                    for(int i = 0; i < N_CHP; i++){
                        p_CHP[s][t][i] = Math.min(Math.max(p_CHP[s][t][i] + random.nextDouble()*(pc_p_CHP[t][i] - p_CHP[s][t][i]), P_CHP_CUR_MIN[s][t][i]), P_CHP_CUR_MAX[s][t][i]);
                        if(t >= 2){
                            if(p_CHP[s][t][i] - p_CHP[s][t-1][i] > 0) p_CHP[s][t][i] = Math.min(p_CHP[s][t][i], p_CHP[s][t-1][i] + UR_CHP[i]);
                            else p_CHP[s][t][i] = Math.max(p_CHP[s][t][i],p_CHP[s][t-1][i]-DR_CHP[i]);
                    ...
                    for(int i = 0; i < N_SPVP; i++){
                        p_SPVP[s][t][i] = Math.min(Math.max(p_SPVP[s][t][i] + random.nextDouble()*(pc_SPVP[t][i] - p_SPVP[s][t][i]), 0), P_SPVP_MAX[t][i]);
                    }
                    for(int i = 0; i < N_WTG; i++){
                        p_WTG[s][t][i] = Math.min(Math.max(p_WTG[s][t][i] + random.nextDouble()*(pc_WTG[t][i] - p_WTG[s][t][i]), 0), P_WTG_MAX[t][i]);
                    }
                    if(!constrainP(s,t)){
                        backPreP(s);
                        flag = false;
        ...
        public void start(String filename) {
            ...
            try {
                foss = new FileOutputStream("PIOData2.txt",true);
    ...
    public static void main(String[] args) {
        for(int eachtime = 0 ; eachtime < 30; eachtime++){
            PIO pio = new PIO(60, 5000, 0.8, 20, 0);
            String filename = "PIOData1.txt";
            pio.start(filename);
        }
    }
}
