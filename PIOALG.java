...
public class PIOALG {
    ...
    public static class PIO{
        ...
        public static void updateV(int its) {
            for (int s = 0; s < popSize; s++) {
                // 更新H\P
                for(int t = 1; t <= 24; t++){
                    for(int i = 0; i < Dim; i++){
                        v[s][t][i] = Math.pow(Math.E, -0.2*its)*v[s][t][i] + random.nextDouble()*(pbest[t][i] - p[s][t][i]);
                    }
                }
            }
        }
        public static void updatePStageOne(){
            for(int s = 0; s < popSize; s++){
                // 更新H
                for(int t = 1; t <= 24; t++){
                    for(int i = N_TGU; i < Dim; i++){
        ...
        public static void updatePStageTwo(){
            ...
            for(int s = 0 ; s < popSize; s++){
                ...
                for(int t = 1; t <= 24; t++){
                    for(int i = 0;i  < Dim; i ++){
                        pc[t][i] += p[s][t][i]*fitVal[s];
                        if(s == popSize - 1) pc[t][i] /= (sum*Np);
                    }
                }
            }
            ...
            for(int s = 0; s < popSize; s++){
                ...
                for(int t = 1; t <= 24; t++){
                    for(int i = N_TGU; i < Dim; i++){
                        p[s][t][i] = Math.min(Math.max(p[s][t][i] + random.nextDouble()*(pc[t][i]-p[s][t][i]), P_H_MIN[s][t][i]), P_H_MAX[s][t][i]);
                    }
                    if(constrainH(s,t)) updateH(s,t);
                    else backPreH(s,t);
                }
                ...
                for(int t = 1; t <= 24; t++){
                    ...
                    int a = 0,b = 0;
                    for(int i = 0; i < Dim - N_HOU; i++){
                        p[s][t][i] = Math.min(Math.max(p[s][t][i] = p[s][t][i] + random.nextDouble()*(pc[t][i]-p[s][t][i]), P_H_MIN[s][t][i]), P_H_MAX[s][t][i]);
                        if(t >= 2){
                            if(i < N_TGU){
                                if(p[s][t][i] - p[s][t-1][i] > 0) p[s][t][i] = Math.min(p[s][t][i], p[s][t-1][i] + UR_TGU[a]);
                                else p[s][t][i] = Math.max(p[s][t][i],p[s][t-1][i]-DR_TGU[a]);
                    ...
                    for(int i = 0,j = 0; i < Dim; i++,j++){
                        ...
                        for(int t = 1; t <= 24; t++){
                            if(t >= 2 && i < N_TGU){
                                ...
                                p[s][t][i] = random.nextDouble(P_H_MAX[s][t][i]*0.5,P_H_MAX[s][t][i]);
                            }
                        }
                    }
                    CHP_FOR(s);
        ...
        public void start(String filename) {
            ...
            try {
                foss = new FileOutputStream("C:\\Users\\Qiu\\Desktop\\EconomicDispatch\\result2\\system4\\bestits.txt",true);
            }catch (Exception e){
    ...
    public static void main(String[] args) {
        for(int eachtime = 0 ; eachtime < 30; eachtime++){
            PIO pio = new PIO(100,1000);
            String filename = "C:\\Users\\Qiu\\Desktop\\EconomicDispatch\\result2\\system4\\data.txt";
            pio.start(filename);
        }
    }
}
