...
public class WOAALG {
    ...
    public static class WOA{
        ...
        public static void updatePStageOne(double a, double c){
            for(int s = 0; s < popSize; s++){
                // 更新H
                for(int t = 1; t <= 24; t++){
                    for(int i = N_TGU; i < Dim; i++){
                        if(i < N_TGU + N_CHP * 2) i++;
                        double d = Math.abs(c*pbest[t][i]-p[s][t][i]);
                        p[s][t][i] = Math.min(Math.max(pbest[t][i] - a*d, P_H_MIN[s][t][i]), P_H_MAX[s][t][i]);
                    }
                ...
                for(int t = 1; t <= 24; t++){
                    ...
                    for(int i = 0; i < Dim - N_HOU; i++){
                        ...
                        if(t >= 2){
                            if(i < N_TGU){
                                if(p[s][t][i] - p[s][t-1][i] > 0) p[s][t][i] = Math.min(p[s][t][i], p[s][t-1][i] + UR_TGU[aa]);
                                else p[s][t][i] = Math.max(p[s][t][i],p[s][t-1][i]-DR_TGU[aa]);
                                a++;
        ...
        public static void updatePStageOnePointFive(double a, double c){
            for(int s = 0; s < popSize; s++){
                ...
                for(int t = 1; t <= 24; t++){
                    for(int i = N_TGU; i < Dim; i++){
                        if(i < N_TGU + N_CHP * 2) i++;
                        double d = Math.abs(c*p[idx][t][i]-p[s][t][i]);
                        p[s][t][i] = Math.min(Math.max(p[idx][t][i] - a*d, P_H_MIN[s][t][i]), P_H_MAX[s][t][i]);
                    }
                    if(constrainH(s,t)) updateH(s,t);
                    else backPreH(s,t);
                }
        ...
        public static void updatePStageTwo(double b, double l){
            for(int s = 0; s < popSize; s++){
                // 更新H
                for(int t = 1; t <= 24; t++){
                    for(int i = N_TGU; i < Dim; i++){
                        if(i < N_TGU + N_CHP * 2) i++;
                        p[s][t][i] = Math.min(Math.max(Math.abs(pbest[t][i]-p[s][t][i])*Math.pow(Math.E,b*l)*Math.cos(2*Math.PI*l) + pbest[t][i], P_H_MIN[s][t][i]), P_H_MAX[s][t][i]);
                    }
                    if(constrainH(s,t)) updateH(s,t);
                    else backPreH(s,t);
                }
        ...
        public void start(String filename) {
            ...
            try {
                foss = new FileOutputStream("C:\\Users\\Qiu\\Desktop\\EconomicDispatch\\result4\\system4\\bestits.txt",true);
            }catch (Exception e){
            ...
            for(int its = 1; its <= T; its++){
                ...
                if(random.nextDouble() < 0.5){
                    ...
                    if(Math.abs(a) < 1) updatePStageOne(a,2* random.nextDouble());
                    else updatePStageOnePointFive(a,2* random.nextDouble());
                }else {
                    updatePStageTwo(1, random.nextDouble(-1,1));
                }
    ...
    public static void main(String[] args) {
        for(int eachtime = 0 ; eachtime < 30; eachtime++){
            WOA woa = new WOA(100,1000);
            String filename = "C:\\Users\\Qiu\\Desktop\\EconomicDispatch\\result4\\system4\\data.txt";
            woa.start(filename);
        }
    }
}
